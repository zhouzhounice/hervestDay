  ...
  <el-header>
    ...
    <div class="right-panel">
      <div class="right-panel-search">
        ...
        <el-button type="primary" style="margin-left:15px" ><el-icon ><Search /></el-icon></el-button>
      </div>
      <el-button v-if="!hideRefresh" @click="refresh" circle style="margin-left:15px"><el-icon ><Refresh /></el-icon></el-button>
      <el-popover v-if="column||1" placement="bottom" title="列设置" :width="500" trigger="click" :hide-after="0" @show="customColumnShow=true" @after-leave="customColumnShow=false">
        <template #reference>
          <el-button circle style="margin-left:5px"><el-icon ><SetUp /></el-icon></el-button>
        </template>
      ...
      <el-popover v-if="!hideSetting" placement="top" title="表格设置" :width="400" trigger="click" :hide-after="0">
        <template #reference>
          <el-button circle style="margin-left:5px"><el-icon ><Setting /></el-icon></el-button>
        </template>
      ...
      <el-button circle style="margin-left:5px" @click="filterBtn">
        <el-icon ><Filter /></el-icon>
      </el-button>
    </div>
  </el-header>
  <slot name="header_serach_item"></slot>
  ...
  <el-main class="nopadding">
    ...
    <div class="scTable-table" :style="{'height':_table_height}">
      <el-table v-bind="$attrs" :data="tableData" :row-key="rowKey" :key="toggleIndex" ref="scTable" :height="height=='auto'?null:'100%'" :size="config.size" :border="config.border" :stripe="config.stripe" :summary-method="remoteSummary?remoteSummaryMethod:summaryMethod" @sort-change="sortChange" @filter-change="filterChange">
        <slot></slot>
        <template >
          <el-table-column v-for="(item, index) in userColumn" :key="index" :column-key="item.prop" :label="item.label" :prop="item.prop" :width="item.width" :sortable="item.sortable" :fixed="item.fixed" :filters="item.filters" :filter-method="remoteFilter||!item.filters?null:filterHandler" :show-overflow-tooltip="item.showOverflowTooltip">
    ...
    <div class="scTable-page" v-if="!hidePagination || !hideDo">
      <div class="scTable-pagination">
        <el-pagination v-if="!hidePagination" background :small="true" :layout="paginationLayout" :total="total" :page-size="scPageSize" :page-sizes="pageSizes" :currentPage="currentPage" @current-change="paginationChange" @update:page-size="pageSizeChange"></el-pagination>
      </div>
...
export default {
  ...
  props: {
    ...
    drawerVisible: { type: Boolean, default: true },
  ...
  mounted() {
    ...
    //判断是否静态数据
    console.log('apiObj=============',this.apiObj  )
  ...
  methods: {
    ...
    filterBtn() {
      ...
      //this.$emit('update:updateParentData', true)
      this.$emit('updateParentData', 'drawerVisible', !this.drawerVisible);
    },
